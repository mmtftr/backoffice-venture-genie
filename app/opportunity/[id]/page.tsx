...
import { AgentActivity, AxisCards, DecisionBanner, MemoView, RecommendationBadge, RescreenButton, SubstitutionLadder, formatDate } from "../../components/OpportunityDetail";
...
export default function OpportunityPage() {
  ...
  return <div className="space-y-6">
    ...
    {!!opportunity.memo?.redFlags.length && <section className="rounded-xl border border-rose-500/30 bg-rose-500/8 p-5"><h2 className="text-sm font-semibold text-rose-300">⚑ Red flags</h2><ul className="mt-2 space-y-1 text-sm text-rose-200/80">{opportunity.memo.redFlags.map((flag) => <li key={flag}>• {flag}</li>)}</ul></section>}
    <DecisionBanner decision={opportunity.decision} />
    <AxisCards opportunity={opportunity} />
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]"><MemoView opportunity={opportunity} memory={memory} /><aside className="space-y-4">
      {opportunity.memo && <section className={`${cardClass} p-5`}><h2 className="text-sm font-semibold text-slate-200">Memo trust</h2><p className="mt-2 font-mono text-3xl text-white">{Math.round(opportunity.memo.trustScore_0_1 * 100)}%</p><p className="mt-1 text-xs text-slate-500">Evidence-weighted claim verification</p></section>}
      {!!opportunity.memo?.gaps.length && <section className={`${cardClass} p-5`}><h2 className="text-sm font-semibold text-slate-200">Information gaps</h2><div className="mt-3 flex flex-wrap gap-2">{opportunity.memo.gaps.map((gap) => <span key={gap} className="rounded-md border border-amber-500/20 bg-amber-500/5 px-2 py-1 text-xs text-amber-200">{gap}</span>)}</div></section>}
      {opportunity.decision && <section className="rounded-xl border border-rose-500/20 bg-slate-900/70 p-5"><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-rose-400">Adversarial view</p><h2 className="mt-1 text-sm font-semibold text-slate-200">Why this could fail</h2><p className="mt-3 text-sm leading-6 text-slate-400">{opportunity.decision.whyThisCouldFail}</p></section>}
      {opportunity.outreachDraft && <section className={`${cardClass} p-5`}><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-emerald-400">Draft only · not sent</p><h2 className="mt-1 text-sm font-semibold text-slate-200">Outbound outreach</h2><p className="mt-3 whitespace-pre-line text-sm leading-6 text-slate-400">{opportunity.outreachDraft}</p></section>}
    </aside></div>
    <AgentActivity summary={opportunity.traceSummary} trace={trace} />
  </div>;
}
